...
func attachWeChatSessionByKey(ctx context.Context, st store.Store, target wechatSendTarget, sessionKey string) (wechatSendTarget, bool, error) {
	...
	// WeChat sessions are owned by each chatter's per-sender app_user
	// (u_xxx), not the channel owner, so enumerate via ListSessions — it
	// widens to sub-users via owner_user_id. LookupSessionTriple's strict
	// WHERE user_id=? would miss chatter-owned rows.
